...
public class potencia {
	...
	public static void vhd(int bits,int n, int k) throws IOException{
		...
		String entity=new String("entity potencia is"+nl+"  port (raiz,potencia:in std_logic_vector("+(bits-1)+" downto 0);"+nl+"       oalfa:out std_logic_vector("+(bits-1)+" downto 0));"+nl+"end potencia;"+nl+nl);
		String architecture="architecture Behavioral of potencia is"+nl+nl;
		...
		architecture+="  oalfa<=\"";for(int i=0;i<bits;i++)architecture+="0";architecture+="\" when conv_integer(raiz)=0 else matriz1(conv_integer(ayuda3));"+nl;
		architecture+="  alfa<=conv_integer(potencia);"+nl;
		architecture+="  contador<=conv_integer(matriz2(conv_integer(raiz)));"+nl+nl;
		architecture+="end Behavioral;"+nl;
		FileWriter write=new FileWriter("potencia.vhd");
